...
def print_detailed_error(exception):    
    ...
    max_label1_width = max([u_othr.get_display_width(label) for label in group1_labels])
    
    # Calculate width of entire group 1
    group1_lines = [
        f" 📄 FILE" + " " * (max_label1_width - u_othr.get_display_width("📄 FILE")) + f" : {filename}",
        f" 📍 LINE" + " " * (max_label1_width - u_othr.get_display_width("📍 LINE")) + f" : {line_number}",
        f" 🔧 FUNCTION" + " " * (max_label1_width - u_othr.get_display_width("🔧 FUNCTION")) + f" : {function_name}"
    ]
    max_group1_width = max([u_othr.get_display_width(line) for line in group1_lines])
    
    # Calculate alignment for GROUP 2 (ERROR TYPE, REASON)
    group2_labels = ["⚠️ ERROR TYPE", "❓ REASON"]
    max_label2_width = max([u_othr.get_display_width(label) for label in group2_labels])
    
    # Calculate width of entire group 2 (only ERROR TYPE and REASON)
    group2_lines = [
        f" ⚠️ ERROR TYPE" + " " * (max_label2_width - u_othr.get_display_width("⚠️ ERROR TYPE")) + f" : {exc_type}",
        f" ❓ REASON" + " " * (max_label2_width - u_othr.get_display_width("❓ REASON")) + f" : {exc_message}"
    ]
    max_group2_width = max([u_othr.get_display_width(line) for line in group2_lines])
    ...
    if has_problem_code:
        ...
        max_group3_width = u_othr.get_display_width(problem_line)
    ...
    for frame in tb:
        ...
        prefix_width = u_othr.get_display_width(prefix)
    ...
    for frame in tb:
        ...
        if frame.line and frame.line.strip():
            padding = max_traceback_prefix_width - u_othr.get_display_width(prefix_clean)
    ...
    for line in lines_to_measure:
        width = u_othr.get_display_width(line)
        if width > max_width:
            max_width = width
    ...
    print_bordered_line(content, u_othr.get_display_width(" 🚨 PROGRAM ERROR"))
    ...
    padding1 = max_label1_width - u_othr.get_display_width("📄 FILE")
    content = label1 + " " * padding1 + f" : {filename}"
    print_bordered_line(content, u_othr.get_display_width(f" 📄 FILE" + " " * padding1 + f" : {filename}"))
    
    label2 = f" 📍 {italic('LINE')}"
    padding2 = max_label1_width - u_othr.get_display_width("📍 LINE")
    content = label2 + " " * padding2 + f" : {line_number}"
    print_bordered_line(content, u_othr.get_display_width(f" 📍 LINE" + " " * padding2 + f" : {line_number}"))
    
    label3 = f" 🔧 {italic('FUNCTION')}"
    padding3 = max_label1_width - u_othr.get_display_width("🔧 FUNCTION")
    content = label3 + " " * padding3 + f" : {function_name}"
    print_bordered_line(content, u_othr.get_display_width(f" 🔧 FUNCTION" + " " * padding3 + f" : {function_name}"))
    ...
    padding4 = max_label2_width - u_othr.get_display_width("⚠️ ERROR TYPE")
    content = label4 + " " * padding4 + f" : {exc_type}"
    print_bordered_line(content, u_othr.get_display_width(f" ⚠️ ERROR TYPE" + " " * padding4 + f" : {exc_type}"))
    
    label5 = f" ❓ {italic('REASON')}"
    padding5 = max_label2_width - u_othr.get_display_width("❓ REASON")
    content = label5 + " " * padding5 + f" : {exc_message}"
    print_bordered_line(content, u_othr.get_display_width(f" ❓ REASON" + " " * padding5 + f" : {exc_message}"))
    ...
    if has_problem_code:
        ...
        print_bordered_line(content, u_othr.get_display_width(f" 🎯 PROBLEM CODE : {code_line.strip()}"))
    ...
    print_bordered_line(content, u_othr.get_display_width(" 🗂️ FULL TRACEBACK:"))
    ...
    for frame in tb:
        ...
        if frame.line and frame.line.strip():
            padding = max_traceback_prefix_width - u_othr.get_display_width(prefix_display)
            code_part = f" → {frame.line.strip()}"
            content = prefix_colored + " " * padding + code_part
            display_width = u_othr.get_display_width(prefix_display + " " * padding + code_part)
            print_bordered_line(content, display_width)
        else:
            content = prefix_colored
            print_bordered_line(content, u_othr.get_display_width(prefix_display))
